...
const DocumentDirection = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      setLoading(true);
      try {
        const res = await axios.get("/api/documents-direction", {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const token = localStorage.getItem("token");
      await axios.delete(`/api/documents-direction/${id}`, { headers: { Authorization: `Bearer ${token}` } });
              ...
              <DataTable columns={["Date", "Nature", "Désignation", "Destinataire", "Priorité", "Ajouté par", "Actions"]} empty="Aucune donnée">
                {current.map(a => (
                  <TR key={a.id}>
                    ...
                    <TD><Badge label={a ? a.priorite : "—"} color={(a.priorite==='Haute'?'danger':'default')} /></TD>

...
const ReglementsEmis = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      setLoading(true);
      try {
        const res = await axios.get("/api/reglements-emis", {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const token = localStorage.getItem("token");
      await axios.delete(`/api/reglements-emis/${id}`, { headers: { Authorization: `Bearer ${token}` } });
  ...
  return (
    <PageLayout>
      <PageHeader
        title="Règlements Émis"
        subtitle={`${filtered.length} règlement émis${filtered.length !== 1 ? "s" : ""} trouvé${filtered.length !== 1 ? "s" : ""}`}
        action={user?.role !== "utilisateur" && <AddButton to="/addReglement" label="Nouveau règlement" />}
      />
      ...
      <Card>
        ...
        <div style={{ padding: "0 20px" }}>
          {loading ? <Spinner /> : (
            <>
              <DataTable columns={["Date saisie", "Code tiers", "Tiers", "Montant brut", "Retenue source", "Montant net", "Ajouté par", "Actions"]} empty="Aucune donnée">
                {current.map(a => (
                  <TR key={a.id}>
                    ...
                    <TD>{(parseFloat(a.montant_brut||0).toLocaleString('fr-FR',{minimumFractionDigits:3})) + ' DT'}</TD>
                    <TD>{(parseFloat(a.montant_retenue_source||0).toLocaleString('fr-FR',{minimumFractionDigits:3})) + ' DT'}</TD>
                    <TD style={{fontWeight:600,color:"#1a202c"}}>{(parseFloat(a.montant_net||0).toLocaleString('fr-FR',{minimumFractionDigits:3})) + ' DT'}</TD>
                    <TD style={{color:"#718096",fontSize:13}}>{a.ajoute_par}</TD>
                    <TD>
                      <div style={{ display:"flex", gap:6 }}>
                        <ViewBtn to={`/detailsReglement/${a.id}`} />
                        <EditBtn to={`/updateReglement/${a.id}`} />
